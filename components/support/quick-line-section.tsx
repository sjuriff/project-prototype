    ...
    <div className="max-w-6xl mx-auto px-6 py-16">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {quickLinks.map((link, index) => (
          <button
            key={index}
            className="bg-tertiary p-8 rounded-lg  transition-colors text-left group"
          >
            <link.icon className="w-10 h-10 text-primary mb-4 group-hover:scale-110 transition-transform" />
            <h3 className="mb-2 font-heading text-tertiary-text">{link.title}</h3>
            <p className="text-secondary font-body">{link.description}</p>
          </button>
        ))}
      </div>
    </div>
